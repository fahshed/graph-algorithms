...
public:
    ...
    void funtion(int ara[][3], int n, int u, int v) ;
};
...
void Graph::funtion(int ara[][3], int n, int u, int v)
{
    dijkstra(u) ;
    Node * uDist = new Node[nVertices] ;
    uDist = wDist ;

    dijkstra(v) ;
    Node *vDist = new Node[nVertices] ;
    vDist = wDist ;
    ...
    for(int i=0; i<n; i++)
    {
        temp = uDist[ara[i][0]-1].weight + ara[i][2] + vDist[ara[i][1]-1].weight ;
        cout << "temp " << uDist[ara[i][0]-1].weight << " " << ara[i][2] << " " << vDist[ara[i][1]-1].weight << endl ;
        if(min > temp)
        {
            min = temp ;
            index = i ;
    ...
    cout << ara[index][0] << " " << ara[index][1] << " " << ara[index][2] << endl ;
}
