...
void Graph::prim()
{
    ...
    int length, position ;
    int mstWeight = 0 ;
    int edges[nVertices-1][2] ;
    bool inQueue[nVertices] ;
    Node pDist[nVertices] ;
    ...
    for(int i=0; i<nVertices; i++)
    {
        pDist[i].weight = INFINITY ;
        pDist[i].vertex = i ;
        pDist[i].from = NULL_VALUE ;
    }
    pDist[s].weight = 0 ;

    MinHeap H ;
    H.heapify(pDist, nVertices) ;
    ...
    {
        u = H.extractMin() ;
        inQueue[u.vertex] = false ;

        if(u.vertex!=s)
        {
            mstWeight += u.weight ;
            edges[indx][0] = u.from ;
            edges[indx][1] = u.vertex ;
            indx++ ;

            g.addEdge(u.from, u.vertex, u.weight) ;
        }
        ...
        for(int i=0; i<length; i++)
        {
            ...
            if(pDist[v.vertex] > v && inQueue[v.vertex])
            {
                pDist[v.vertex].weight =  v.weight ;
                pDist[v.vertex].from = v.from ;

                position = H.heapIndex(v) ;
                H.decreaseKey(position, pDist[v.vertex]) ;
            }
        }
    }
